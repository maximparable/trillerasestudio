...
document.getElementById('generate-cache-btn').addEventListener('click', () => {
  fetch('/generate_manifest', { method: 'POST' })
    ...
    .catch(error => {
      alert('Error: ' + error + "\n¡Esta opción sólo funciona en el servidor local!\nAsegúrese de que su servidor esté activo.");
    });
});
...
function CheckFW() {
  ...
  if (ps4Regex.test(userAgent)) {
    ...
    document.getElementById('PS4FW').textContent = `No estás en una plataforma PS4: ${platform}`;
    document.getElementById('PS4FW').style.color = 'yellow';
  ...
  try {
    ...
    if (localStorage.getItem('HEN')) {
      if (JailbreakModule && typeof JailbreakModule.HEN === 'function') {
          ...
          console.error("Función HEN no encontrada en el módulo Jailbreak.js");
      }
    } else if (localStorage.getItem('GoldHEN')) {
      if (JailbreakModule && typeof JailbreakModule.GoldHEN === 'function') {
          JailbreakModule.GoldHEN();
      } else {
          console.error("La función GoldHEN no se encuentra en el módulo Jailbreak.js");
      }
    } else {
      if (JailbreakModule && typeof JailbreakModule.GoldHEN === 'function') {
          JailbreakModule.GoldHEN();
    ...
    console.error("No se pudo hacer jailbreak:", e);
  }
}
...
async function binloader() {
  try {
    ...
    if (goldhenModule && typeof goldhenModule.runBinLoader === 'function') {
      ...
      console.error("La función GoldHEN no se encuentra en el módulo GoldHEN.js");
    }
  } catch (e) {
    console.error("No se pudo hacer jailbreak:", e);
  }
...
async function Loadpayloads(payload) {
  try {
    ...
    if (payloadModule && typeof payloadModule[payload] === 'function') {
      ...
      console.error(`${payload} Función no encontrada en el módulo payloads.js`);
    }
  } catch (e) {
    console.error(`No se pudo cargar ${payload}:`, e);
  }
}
